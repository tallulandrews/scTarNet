plotDependencies <- function (combined_dependencies, recurr_threshold=max(combined_dependencies$recurr)/2, suppress.plot=FALSE, seed.col="dodgerblue", target.col="goldenrod1", edge.col=c("blue", "black", "red"), rnd.seed=2948) {
	set.seed(rnd.seed)
	# TFs -> Targets Graph
	...
	invisible(list(edgelist=AdjList, graph=Graph_to_plot));
}


plotInteractions<-function(combined_interactions, recurr_threshold=max(combined_interactions$pair2consistent[,3])/2, target_threshold=1, suppress.plot=FALSE, seed.col="dodgerblue", interaction.col="black", rnd.seed=2948) {
	set.seed(rnd.seed)
...
# Include pathways?
plotInteractionsWithTargets <- function (combined_dependencies, combined_interactions, recurr_threshold=max(combined_interactions$pair2consistent[,3])/2, suppress.plot=FALSE, seed.col="dodgerblue", target.col="goldenrod1", interaction.col="black", edge.col=c("blue", "grey45", "red"), rnd.seed=2948) {
	set.seed(rnd.seed)
	...
	interactions <- interactions$edgelist;
	...
	dependencies <- dependencies$edgelist;
	...
	Node_colour <- node_cols[as.numeric(V(Graph)$name %in% as.character(dependencies$Gene))+1]
	...
	invisible(list(edgelist=AdjList, graph=Graph))
